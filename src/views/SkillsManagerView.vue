...
function removeSkill(s: SkillConfig) { dialog.warning({ title: '确认删除', content: `确定删除「${s.name}」？（同时删除文件）`, positiveText: '删除', negativeText: '取消', onPositiveClick: async () => { try { await api.delete(`/skill-config/${s.id}`); message.success('已删除'); await fetchSkills() } catch { message.error('删除失败') } } }) }
    ...
    <div class="page">
      ...
      <template v-if="!categoryId">
        <div class="page__header animate-slideIn">
          <div class="page__header-text"><h1 class="page__title">技能管理</h1><p class="page__sub">按分类浏览 Skills · 共 {{ categories.length }} 个分类</p></div>
        </div>
      ...
      <template v-else>
        ...
        <div class="page__content animate-slideIn" style="animation-delay:50ms">
          <n-spin :show="loading">
            <div v-if="skills.length === 0 && !loading" class="empty"><span class="empty__icon">📋</span><p class="empty__title">此分类下还没有技能</p></div>
            <div class="grid">
...
.cat-card { background: var(--card-bg); border: 1px solid var(--border-subtle); border-radius: 14px; padding: 24px 20px; cursor: pointer; transition: all .15s ease; }
.cat-card:hover { border-color: var(--accent-primary); box-shadow: 0 4px 16px rgba(0,0,0,.08); transform: translateY(-1px); }
.cat-card__name { font-size: 1.125rem; font-weight: 700; color: var(--text-primary); margin-bottom: 6px; }

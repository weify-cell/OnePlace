...
async function fetchCategories() {
  loading.value = true
  try { const { data } = await api.get('/tool-category/list'); categories.value = data } catch { message.error('加载失败') }
  loading.value = false
}
...
async function fetchTools() {
  ...
  loading.value = true
  try { const { data } = await api.get('/tool-config/list', { params: { category_id: categoryId.value } }); tools.value = data } catch { message.error('加载失败') }
  loading.value = false
}
...
function openCreate() {
  editing.value = null
  form.value = { name: '', label: '', description: '', instruction: '', enabled: 1, category_id: categoryId.value }
...
async function saveTool() {
  ...
  try {
    form.value.category_id = categoryId.value
      ...
      <template v-if="!categoryId">
        <div class="page__header animate-slideIn">
          <div class="page__header-text">
            ...
            <p class="page__sub">按分类浏览 Agent 工具 · 共 {{ categories.length }} 个分类</p>
          </div>
        </div>
        ...
        <div class="page__content animate-slideIn" style="animation-delay:50ms">
          <n-spin :show="loading">
            <div v-if="tools.length === 0 && !loading" class="empty">
              ...
              <p class="empty__title">此分类下还没有工具</p>
              <p class="empty__desc">点击「新建工具」添加</p>
            </div>
            <div class="grid">

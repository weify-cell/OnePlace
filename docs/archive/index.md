...
## v1.10 | 代办模块 - 状态Tab重构 | 2026-04-02

- 主要功能：5个Tab标签切换（全部/待办/进行中/已完成/已取消）、已完成记录`completed_at`、排序规则优化、登录弹窗提醒（每天一次）
- 目录：docs/archive/v1.10-todo-status-tabs/
- 结论：全部 5 项 AC 通过，排序规则与 PRD 一致（降序），验收通过

------

## v1.9 | 笔记模块 - 预览/编辑双模式 | 2026-04-02

- 主要功能：笔记详情页预览/编辑双模式（默认预览，点击标题或编辑按钮进入分屏编辑）、笔记列表卡片/列表切换并持久化到 localStorage
- 目录：docs/archive/v1.9-notes-preview-mode/
- 结论：全部 8 项 AC 通过，BUG-011 已修复，回归测试通过，验收通过

------

## v1.8 | 笔记模块 - Markdown 富文本编辑 | 2026-04-01

- 主要功能：CodeMirror 6 Markdown 编辑器 + 实时预览分屏、图片粘贴/拖拽上传、图片历史管理、存量 Tiptap 笔记迁移
- 目录：docs/archive/v1.8-notes-markdown/
- 结论：全部 8 项 AC 通过，BUG-010 已修复，验收通过

---

## v1.7 | 百宝箱 - 文本比较工具交互重构（内联高亮） | 2026-03-29

- 主要功能：重构文本比较工具交互，取消底部差异面板，改为输入区内联高亮展示（原文红色背景+删除线，对比文绿色背景）
- 目录：docs/archive/v1.7-toolbox-text-diff/
- 结论：全部 6 项 AC 通过，Bug-008 已修复，验收通过
